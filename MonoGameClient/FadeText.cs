...
namespace GameComponentNS
{
    class FadeTextManager : DrawableGameComponent
    {
        ...
        {
            //what this does looks through each component and sees what types in the assembly
            //foreach fader
            var faders = Game.Components.Where(t => t.GetType() == typeof(FadeText)); // gets us a collection of faders, if >0 work out all of the positions
            if(faders.Count() > 0)
            {
